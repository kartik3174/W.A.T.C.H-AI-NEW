...
export default function AnimalCarePage({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
  const { t } = useLanguage()
  const { id: animalId } = use(searchParams)
  ...
  return (
    <div className="container mx-auto py-6 relative">
      ...
      <PageHeader 
        title={t("animal.care.title") || "Animal Care"} 
        description={t("animal.care.description") || "Health monitoring and care recommendations"} 
      />
      <AnimalCareHub />
    </div>
  )
