          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
            ...
            <div className="flex gap-2 w-full sm:w-auto">
              ...
              >
                ...
                {isRefreshing ? t("refreshing") || "Refreshing..." : t("refresh.data") || "Refresh Data"}
              </Button>
      ...
      <Tabs defaultValue="live-feed" value={activeTab} onValueChange={setActiveTab} className="space-y-6">
        <TabsList className="w-full grid grid-cols-2 md:grid-cols-4 h-auto bg-card border-border">
          ...
          <TabsTrigger value="insights" className="py-2 text-muted-foreground data-[state=active]:bg-secondary data-[state=active]:text-primary">
            {t("tab.insights") || "Predictive Insights"}
          </TabsTrigger>
          <TabsTrigger value="timeline" className="py-2 text-muted-foreground data-[state=active]:bg-secondary data-[state=active]:text-primary">
            {t("tab.timeline") || "Activity Timeline"}
          </TabsTrigger>
        </TabsList>
      ...
      <Card className="border-border bg-card">
        <CardHeader>
          <CardTitle className="flex items-center gap-2 text-card-foreground">
            ...
            {t("analytics.card.title") || "Comprehensive Analytics"}
          </CardTitle>
          <CardDescription className="text-muted-foreground">{t("analytics.card.subtitle") || "View behavioral patterns and health trends"}</CardDescription>
        </CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground mb-4">
            {t("analytics.card.description") || "Access detailed behavioral trends analysis, health status distribution, and comprehensive animal health profiles in the dedicated Analytics module."}
          </p>
          <Button
            asChild
          ...
          >
            <Link href="/analytics" className="flex items-center justify-center gap-2">
              {t("view.analytics") || "View Analytics"}
