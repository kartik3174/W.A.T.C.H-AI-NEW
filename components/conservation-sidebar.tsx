...
const advancedNavItems: NavItem[] = [
  { name: "Action View", href: "/action-view", icon: Eye },
  { name: "Drone Response", href: "/drone-response", icon: Cpu },
  { name: "Anti-Poaching", href: "/anti-poaching", icon: Siren },
  { name: "Research", href: "/research", icon: Brain },
  { name: "Disaster Management", href: "/disaster-management", icon: AlertCircle },
]
...
export function ConservationSidebar() {
  ...
  return (
    <div className="fixed left-0 top-0 h-screen w-64 border-r border-border bg-background/95 backdrop-blur-sm shadow-lg z-30">
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-border h-16" style={{ background: "var(--header-gradient)" }}>
      ...
      <div ref={navRef} className="h-[calc(100vh-12rem)] overflow-y-auto p-4 scrollbar-thin" onScroll={checkScroll}>
        <nav className="space-y-6">
          <div>
            <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">{t("Main")}</h2>
            <ul className="space-y-1">
              {[
                { name: t("dashboard"), href: "/dashboard", icon: Home },
                { name: t("tracking"), href: "/tracking", icon: Map },
                { name: t("animal.care"), href: "/animal-care", icon: Heart },
                { name: t("ai.monitoring"), href: "/ai-monitoring", icon: Brain },
              ].map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className={`flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-colors ${
                      pathname === item.href || pathname.startsWith(item.href.split('/')[1] !== undefined ? '/' + item.href.split('/')[1] : item.href)
          ...
          <div>
            <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">{t("Tools")}</h2>
            <ul className="space-y-1">
              {[
          ...
          <div>
            <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">{t("Advanced")}</h2>
            <ul className="space-y-1">
              {[
                { name: t("action.view"), href: "/action-view", icon: Eye },
                { name: t("drone.response"), href: "/drone-response", icon: Cpu },
                { name: t("anti.poaching"), href: "/anti-poaching", icon: Siren },
                { name: t("research"), href: "/research", icon: Brain },
                { name: t("disaster.management"), href: "/disaster-management", icon: AlertCircle },
              ].map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
          ...
          <div>
            <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">{t("Account")}</h2>
            <ul className="space-y-1">
              {[
      ...
      <div className="absolute bottom-0 left-0 right-0 border-t border-border bg-background/95 flex flex-col">
        {userEmail && (
          <div className="px-4 py-2 border-b border-border bg-[#14532D]/30">
            <p className="text-[10px] text-muted-foreground uppercase font-bold truncate">{t("logged.in.as")}</p>
        ...
        <div className="p-4 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">{t("hub.name")}</span>
            </div>
